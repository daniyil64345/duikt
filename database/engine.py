...
async def list_db():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        for table in tables:
            ...
            async with db.execute(f"SELECT rowid as id, * FROM '{table_name}'") as cursor2:
                ...
                for row in rows:
                    try:
                        ...
                        name = row["Назва товару"] if "Назва товару" in row.keys() else "Немає назви"
                        price = row["Ціна"] if "Ціна" in row.keys() else "0"
                        products.append((row_id, name, price))
                    except (KeyError, IndexError) as e:
                        print(f"Помилка при обробці рядка в таблиці {table_name}: {e}")
                        continue
...
async def show_something(category: str):
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        try:
            async with db.execute(f'SELECT rowid as id, * FROM "{category}"') as cursor:
        ...
        for row in rows:
            try:
                ...
                name = row["Назва товару"] if "Назва товару" in row.keys() else "Немає назви"
                price = row["Ціна"] if "Ціна" in row.keys() else "0"
                photo = row["photo_id"] if "photo_id" in row.keys() else "Немає фото"
                quantity = row["Кількість"] if "Кількість" in row.keys() else "0"
...
async def add_photo(data, image):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(
        f"UPDATE '{category}' SET photo_id = ? WHERE id = ?",
            (image, id)
        )
...
async def buy(category:str,  id:int):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if quantity >1:
            await db.execute(f"UPDATE '{category}' SET 'Кількість'= ? WHERE id = ?",
                         (quantity - 1, id)
                        )
        ...
        await db.commit()
        print(f"✅ Оновлено товар {name}: нова кількість {max(quantity - 1, 0)}")
...
async def create_category(category: str):
    ...
    forbidden = {"select", "from", "table", "insert", "delete", "update", "where", "join", "drop", "create"}
    if category.lower() in forbidden:
        raise ValueError(f"'{category}' — це зарезервоване слово SQL, обери іншу назву!")
...
async def update_info(data):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(f"""UPDATE "{category}"
                         SET {edit} = ?
                         WHERE "Назва товару" = ?
                         """, (value, product_name))
        await db.commit()
